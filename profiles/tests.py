...
def create_profile(name, position, institution, grad_date, country_id):
	return Profile.objects.create(name=name, position=position, institution=institution, grad_date=grad_date, country_id=country_id)
...
class ProfileIndexViewTests(TestCase):
    def test_no_profile(self):
        ...
        """
        response = self.client.get(reverse('viewlist:index'))
...
class QuestionDetailViewTests(TestCase):
    def test_view_profile(self):
        ...
        """
        new_profile = create_profile('New Profile', 'Tester', 'My University', '2007-06-27', 24)
        url = reverse('viewlist:detail', args=(new_profile.id,))
